...
class Particle(object):
    ...
    def correction_step(self, sensor_measurements):
        ...
        for measurement in sensor_measurements:
            ...
            #ADDED 
            particle_x      = robot_pose[0]
            particle_y      = robot_pose[1]
            particle_theta  = robot_pose[2]
            ...
            if not landmark.observed: 
                ...
                landmark_x = particle_x + (sensor_measurements[i]).z_range * np.cos(particle_theta+(sensor_measurements[i]).z_bearing)
                landmark_y = particle_y + (sensor_measurements[i]).z_range * np.sin(particle_theta+(sensor_measurements[i]).z_bearing)
                landmark.mu = [landmark_x, landmark_y]
                ...
                error=np.array([sensor_measurements[i].z_range-expected_z[0], normalize_angle(angle_diff(sensor_measurements[i].z_bearing, expected_z[1]))])
                

                # TODO: update the mean and covariance of the EKF for this landmark
                landmark.mu = landmark.mu + K.dot(error)
                ...
                f = 1/np.sqrt (np.linalg.det(2*pi*Q_t))
                expo = -0.5*np.dot(error.T, np.linalg.inv(Q_t)).dot(error)
                self.weight = (self.weight)*f*np.exp(expo)
            
            
            if (i < len (sensor_measurements)):
                i = i+1
            else: 
                break
...
class Plotter(object):
    ...
    def fast_slam(self, t):
        ...
        # for particle in self.particles:
        #     particle.weight = particle / normalizer
